...
pub fn effective_total_memory() -> u64 {
    ...
    // walks the ancestor cgroup tree to find inherited limits, which correctly
    // handles cases like Zoox where MemoryMax is set on a parent slice rather
    // than directly on the process's own cgroup.
...
mod tests {
    ...
    fn test_effective_total_memory_falls_back_to_system_memory() {
        ...
        // Check if a cgroup limit is active using the same method as the implementation:
        // Process::cgroup_limits() instead of System::cgroup_limits().
        let pid = Pid::from_u32(std::process::id());
        sys.refresh_processes_specifics(
            sysinfo::ProcessesToUpdate::Some(&[pid]),
            false,
